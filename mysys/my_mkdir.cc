...
int my_mkdir(const char *dir, int Flags, myf MyFlags) {
  ...
  EasyLoggerWithTrace(multi_master::path_log_mysys, EasyLogger::info).force_flush() << "my_mkdir::mkdir. " << dir;
...
#ifdef MULTI_MASTER_ZHANG_LOG
  EasyLoggerWithTrace(multi_master::path_log_mysys, EasyLogger::info).force_flush() << "my_mkdir::mkdir. " << dir << ", ret:" << ret;
#endif // MULTI_MASTER_ZHANG_LOG
  if(ret != 0)
#else
